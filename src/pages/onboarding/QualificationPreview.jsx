...
const QualificationPreview = () => {
  ...
  const initialFormData = location.state?.formData
    ...
    : [];
  const [qualifications, setQualifications] = useState(initialFormData);
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="w-1/2 p-12">
        ...
        <h1 className="text-2xl font-semibold text-gray-900">Qualification</h1>
        <p className="text-sm text-gray-500 mt-1">
          Include all of your qualifications in this section.
        </p>

        {qualifications.length > 0 ? (
          qualifications.map((formData, index) => (
            <div key={index} className="mt-6 bg-gray-50 p-6  relative">
              ...
              <ul className="list-disc list-inside mt-3 text-gray-700 text-sm space-y-1">
                ...
                {formData.courseType && (
                  <li>Course Type: {formData.courseType}</li>
                )}
          ...
          <p className="mt-6 text-gray-500">No qualifications added yet.</p>
        )}
        <button
          onClick={handleAddAnother}
          className=" mt-7 flex items-center space-x-2 text-black hover:text-blue-700 transition"
        >
          <PlusCircle size={20} /> <span>Add Qualification</span>
        </button>
        {/* Buttons */}
